...
def train_test_split(rows, test_size=0.2, random_seed=42):
...
def main():
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        list(zip(X, y)), test_size=0.2, random_seed=42
    )
    X_train = [x for x, _ in X_train]
    y_train = [v for _, v in X_train]
    X_test = [x for x, _ in X_test]
    y_test = [v for _, v in X_test]
